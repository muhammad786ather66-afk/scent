...
(function () {
  ...
  function addToCart(productId, quantity = 1, showToast = true) {
    ...
    if (showToast) {
      ...
      const title = product ? product.name : 'Fragrance';
      showCartNotification(`${title} added to your bag.`);
    }
  ...
  function getCartSummary() {
    ...
    const enrichedItems = cart.map(item => {
      const product = window.getVeloraProductById ? window.getVeloraProductById(item.id) : null;
      totalCount += item.quantity;

      const itemSubtotal = (product && typeof product.numericPrice === 'number' && product.numericPrice > 0)
        ? (product.numericPrice * item.quantity)
        : 0;
      ...
      return {
        ...
        lineTotalDisplay: itemSubtotal > 0 ? `PKR ${itemSubtotal.toLocaleString()}` : (product ? product.price : 'PKR 0,000'),
        product: product || {
          id: item.id,
    ...
    const subtotalDisplay = hasCustomPrices 
      ? `PKR ${numericSubtotal.toLocaleString()}` 
      : 'PKR 0,000';
